...
- 结果按完全匹配、包含关键词、其他三档排序，第一条候选显示书源、搜索耗时和已聚合书源数；结果列表按终端高度滚动。
- 搜索页实时显示 `已完成/总书源数`、当前书源和候选数量；搜索中按 `Esc` 取消但留在结果页，保留已返回结果，只有非搜索状态下的 `Esc` 才返回上一页。
- 详情页显示当前书源、搜索耗时、最新章节和更新时间，并提供开始/继续阅读、章节列表、书源切换和加入书架入口。
- 详情、目录、正文、已知书源和辅助页均按终端高度内部滚动；书架和搜索结果项可按 `o` 打开统一操作菜单。
...
| 搜索 | 输入关键词，`Enter` 提交 |
| 列表 | `j/k`、方向键、`PageUp/PageDown` 移动，`Enter` 打开，`o` 操作菜单 |
| 详情 | `Enter` 开始/继续阅读，`t` 目录，`s` 书源，`a` 书架，`o` 操作菜单 |
| 目录 | `j/k`、分页键移动，`Enter` 阅读，`o` 操作菜单 |
| 阅读 | `j/k` 滚动，空格翻页，`b` 回退，`[`/`]` 切章，`t` 目录，`s` 书源，`a` 书架，`o` 操作菜单 |
| 已知书源 | `Enter` 使用本地候选，`m` 手动搜索更多，搜索期间 `j/k` 滚动增量结果，`o` 操作菜单 |

`o` 菜单固定提供“继续阅读、书籍信息、章节列表、书源切换”。当前页面或普通异步操作不允许的动作会显示禁用原因；搜索中已返回的候选仍可操作，执行时会先取消剩余来源；底部键位栏始终提示当前页面可执行动作。
