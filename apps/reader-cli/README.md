...
- 目录可按 `/` 输入章节标题筛选，支持 NFKC 和大小写不敏感匹配；焦点选择与当前阅读章节分离。
...
- 已知书源页只读本地候选，当前书源只显示“当前”；`t` 预览选中书源目录，`Enter` 保留现有切换/章节映射行为。直接阅读非当前书源的章节时，正文成功后会更新活动书源和阅读记录。
...
| 目录 | `j/k`、上下方向键移动，`/` 搜索章节，`Esc` 清除筛选或返回，`Home/End` 首尾，`Enter` 阅读 |
