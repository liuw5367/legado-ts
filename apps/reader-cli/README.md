...
- 阅读页显示当前可视页/总页数和章节非空白字数；上下箭头或 `j/k` 翻页，左右箭头或 `h/l` 切换章节，`r` 刷新当前正文。
...
| 详情 | `Enter` 阅读，`t` 目录，`s` 书源，`a` 书架 |
| 目录 | `j/k`、上下方向键移动，`/` 搜索章节，`Esc` 清除筛选或返回，`Home/End` 首尾，`Enter` 阅读 |
| 阅读 | 上下方向键或 `j/k` 翻页，左右方向键或 `h/l` 切换章节，`PageUp/PageDown` 或空格也可翻页，`t` 目录，`s` 书源，`a` 书架，`r` 刷新正文 |
| 已知书源 | `t` 预览目录，`Enter` 切换/继续或进入章节映射，`m` 搜索更多 |
