...
核心参考、流程、指南和集成文档必须在正文中写出理解当前主题所需的最小契约：输入字段及所有权、处理顺序、输出结构、合法空结果、状态变化、错误、取消、超时、清理和提交边界。引用源码、测试、外层 API 或其他文档时，引用只承担以下职责：

- 说明 Android 事实、版本差异或证据位置；
- 提供对比实现、测试定位和排查入口；
- 链接到一个已经在当前主题中以最小结构说明过的共享事实源。

读者不打开引用目标时，也必须能够完成当前主题的实现或审查。主题文档不复制无关领域的全部模型，但不能用“完整定义见另一文件”省略本主题的必要字段和失败语义。目录 README 只负责导航；质量文档可以以证据索引为主，但 fixture 和断言仍须定义本地最小结构。
