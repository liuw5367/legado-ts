...
ContentIdentity 使用上表的身份字段；ContentSaveToken 包含 key（ContentIdentity）、operationId 和 writeVersion，类型定义见 [正文](09-content-flow.md)。Android folderName 仅留在文件 adapter。单进程锁只保证单进程；多实例需跨实例原子比较写入。
...
预览使用独立临时命名空间，不自动提交用户状态。真实网络本身仍可能产生上游副作用，因此预览不自动触发购买、评论写入或交互事件。测试见 [状态及交叉案例](15-conformance-tests.md#具体状态与交叉案例)。
