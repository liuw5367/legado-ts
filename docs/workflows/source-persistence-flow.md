...
Android 的 Web API 是兼容适配层：普通 `/saveBookSource` 直接保存单条有效数据，批量 `/saveBookSources` 会跳过无效数组成员；这与 package 的“先预览、再确认、按事务提交”推荐流程不同，详见[Legado Web API 适配](../integration/legado-web-api-bridge.md)。
