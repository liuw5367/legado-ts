...
每增加宿主方法，同步 11、13 与具体案例。CAP-ARCHIVE、CAP-FONT、加密工厂返回对象及应用绑定的方法按 19 的清单逐重载实现；原文保留不等于运行支持。归档验证路径/大小限制，字体验证轮廓映射，锁验证同源互斥与释放。动态登录和段评写入的阻塞项必须先确定参数与副作用，不能根据字段名猜测。
