...
三者可以由同一 `RuleSub` 订阅下载，但解析、保存、启用、冲突和运行时结果分别建模。`RuleSub.type` 的兼容映射见[书源订阅与刷新](../flows/source-subscriptions.md)。
...
刷新比较结果由[订阅流程](../flows/source-subscriptions.md)的 `SubscriptionRefreshPlan` 承载；`SubscriptionRefreshAttempt` 只记录一次任务的生命周期和结算状态，不能再次声明 `updated/conflict/stale` 等比较结果。
