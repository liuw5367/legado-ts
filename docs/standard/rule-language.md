...
export interface RuleVariableView {
  ...
  /** 判断变量是否可命中；local 按键存在判断，其他作用域按 Android 的空值回退规则判断。 */
...
默认写入目标和读取顺序必须与 Android 一致：写入优先尝试当前章节、当前书籍、当前规则数据，最后写入书源变量；读取先查 `local`，再查章节变量、书变量、规则数据变量和书源变量。`local` 是一次规则调用的局部绑定，键存在即立即返回，即使值是空字符串；章节、书籍、规则数据和书源变量才会在空字符串时继续查找下一层。全部没有结果时返回空字符串给 `@get` 和 `{{}}`。`bookName` 和 `title` 是保留名称，分别映射当前书名和章节标题。`set` 使用 `null` 表示删除，`snapshot` 不得包含 Cookie、Token、密码或 Authorization。
...
JSONPath 解析失败时，`getString`、`getStringList` 和规则链中的 JSONPath 读取会记录错误并返回空字符串/空列表；`getObject`/`getElement` 直接读取对象时异常会继续向上抛出，因此 `init` 等必需字段可能使流程失败。对 `NativeObject`，字符串列表入口只执行第一条规则，`Mode.Json` 读取列表，带多个参数时保留规则文本用于 `{{}}`，否则直接按键访问；`LinkedTreeMap` 也直接按首条规则取键。不能把这些入口统一归为空值。

## 7. Regex 规则和 `$n`

两种正则语义必须分开：

1. `:regex` 全规则模式：`AnalyzeByRegex.getElements` 为每次匹配返回一个列表，列表第 0 项是完整匹配文本，后续依次是捕获组；不存在的捕获组以空字符串占位。`getElement` 只返回首个匹配，但通过 `group(...)!!` 读取捕获组，未参与匹配的可选组可能抛异常。两者都支持多个正则串联，后一个正则作用于前一个正则拼接出的完整匹配文本，不作用于前一个正则的各个捕获组。该语法主要给书籍列表和目录列表使用。
...
表达式求值失败（JS 报错、规则非法）时该片段展开为空串，整条规则不因此失败。

**插值会改变规则模式。** 规则体里出现 `{{...}}` 或 `@get:{}` 时（`AnalyzeRule.kt` 的 mode 切换分支），Android 把它切到 Regex 模式，结果是**插值后的文本本身**，不再当选择器、JSON 路径或 XPath 解析：
`https://book.zri.moe/api/book/{{$.id}}` 得到 `https://book.zri.moe/api/book/42`，`<p>{{$.x}}</p>` 得到 `<p>正文</p>`。
只有 `@js:`/`<js>` 例外：它们先插值再执行 JS。`##` 之后的匹配串与替换串同样参与插值（Android 先对整条规则插值再按 `##` 切分）。
插值结果为空串时按 Android 的两条路径分开处理：**列表规则**走 `getStringList`，`if (rule.isNotEmpty())` 不成立时沿用分析前的整页内容；**字段规则**走 `getString`，插值为空等价于 `AnalyzeByJSoup.getString("")`，结果是空串（`@get:{未定义变量}` 因此不会再拿到整页内容）。只剩 `@put:{...}` 的空规则串同理。

变量查找优先级是：当前规则局部变量、章节变量、书变量、通用规则数据变量、书源变量。局部变量命中后不再回退，其他作用域的空字符串继续查找；`bookName` 和 `title` 是保留含义，分别优先映射书名和章节标题。`@put` 写入后，后续规则步骤和同一流程可以读取；流程结束后的持久化结果由宿主提交，规则引擎不能自行开启数据库事务。

### `##match##replace`

字段结果支持在末尾附加正则替换：`规则##匹配##替换`。第三段可以省略，`规则##匹配` 等价于使用空字符串替换。未设置第四段时，对当前规则结果执行全量替换；存在第四段时，Android 先取匹配到的第一个完整片段，再只在这个片段上替换，原结果中片段前后的文本不会保留。没有匹配时返回空字符串；编译失败时，非首次分支回退为字面字符串全量替换，首次分支返回 replacement。替换发生在当前规则步骤结果生成之后；正文的 `replaceRegex` 则是在多页正文合并后再次执行。

当前实现编译正则失败时，普通替换分支对全量结果回退为字面字符串替换，首次替换分支直接返回 replacement；迁移必须以测试固定这一历史行为，不应直接依赖 JavaScript `String.replace` 的差异语义。
...
`java` 绑定含 `openUrl`、`openVideoPlayer`、`startBrowser` 等 UI 跳转方法。宿主启用 `blockSourceNavigation` 且当前协程上下文携带 `SuppressSourceNavigation`（换源、批量检测等后台流程）时，这些跳转和 WebView 弹窗会被阻止；Android 当前行为是静默 `return`，JS 仅得到 `undefined`（`JsExtensions.kt` L1251/L390/L405），`policy-denied` 诊断是迁移设计，映射示例见 [运行时统一契约](../implementation/runtime-contracts.md)。TypeScript 宿主应输出空结果或显式的策略拒绝，不能弹出书源网页或视频。
