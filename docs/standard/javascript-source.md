...
规则和源脚本可见绑定包括 `java`、`source`、`sourceApi`、`baseUrl`、`cookie`、`cache`，以及 `key`、`page`、`book`、`chapter`、`chapters`、`result`、`nextChapterUrl` 等流程变量。绑定名称和对象角色是兼容契约，脚本可调用方法、返回值、异常和能力开关见 [宿主接口](../implementation/runtime-host-interfaces.md#javascript-书源宿主-api)。
...
`getReviewDetail` 返回评论项分页对象，至少包含 `items` 数组，可以包含 `nextPageUrl`；函数缺失或调用异常属于能力/脚本错误，函数返回空值、无法解析的对象或缺少 `items` 数组时该详情页结果为空。Android 只从评论项顶层读取 `id`、`avatar`、`name`、`badge`、`content` 和 `replies`，顶层的 `replyToName`、`img`、`audio`、`time`、`likeCount`、`replyCount` 不会被读取。每个项目必须有非空 `content`，仅有昵称或缺少 `content` 的项目会被丢弃；图片、音频及互动字段必须放在 `content` 协议对象中。`content` 如果是包含 `text`、`replyToName`、`img`、`audio`、`time`、`likeCount` 或 `replyCount` 的协议对象，运行时会解析协议；JS 源中的图片、音频和头像相对 `chapter.url` 转换为绝对地址，声明式规则则相对 HTTP 响应最终 URL 解析。若协议对象的七个字段全部为空，`parseContentProtocol` 返回 null，此时对象 JSON 会被当作纯文本 `content` 保留。对象型 `content` 经 `optContent` 转为 JSON 字符串后只要非空即有效，最终解析出的纯文本可为空；仅含 `img`/`audio` 的协议内容会保留（`JsSourceReviewTest` 断言 `reply.content == ""` 但 `imageUrl` 保留）。

`getReviewReplies` 返回 `{ items }`，没有独立的 `nextPageUrl` 字段；运行时会把嵌套 `replies` 展平成回复列表，并清除回复项继续嵌套的结构。函数未声明时返回合法空页并保留能力诊断，返回空值时返回空列表；返回值不是对象、缺少 `items` 或 `items` 不是数组时属于返回格式错误。回复函数只有在摘要和详情函数都存在时才允许导入，调用时由宿主传入 `reviewId`，不是由脚本自行从全局变量读取。

导入校验必须区分以下情况：段评函数声明存在但不是函数、摘要函数缺失、摘要存在但详情缺失、回复函数单独存在。摘要和详情构成一对基本能力，回复是可选的第三层能力；声明不完整时导入失败。运行时还要区分可选摘要/回复函数缺失与必需详情函数缺失：前者可返回空结果并带诊断，后者在调用时返回明确的 capability error，不能把错误伪装成空评论成功。
