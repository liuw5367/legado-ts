...
3. 调用 [公开 package 入口](18-package-usage.md)；转发可安全展示的进度与结构化错误，保留书源级失败与流程级失败的区别。
...
测试 ID 和证据等级使用 [一致性测试基线](15-conformance-tests.md)。设计新增的行为，例如没有 Android 执行证据的段评写入，单独建立 Web 目标测试，不作为 Android 等价 golden。正式发布前对已支持的书源能力运行全部对应 fixture，并对目标部署执行最小端到端操作；尚未实现的能力在 [兼容性矩阵](13-compatibility-matrix.md) 中保留状态。
