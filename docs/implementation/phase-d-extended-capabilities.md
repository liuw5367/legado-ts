...
下列第 1 至 3 项由 B/C 设计和验证；在 runtime 尚未实现时不能标记为已交付。本阶段复用并回归验证；第 4 项批量为本阶段新增。基础脚本能力不能等到 D 才提供。
...
段评读取先实现结构化摘要、详情分页和回复，声明式规则与 JS 函数各走自己的输入路径；同时保留 `getDP/getZP` 旧式网页桥接及其 nonce 会话、CSP/sandbox 和脚本回调边界。结构化入口统一输出段落索引、评论 ID、内容协议和分页状态，旧式入口返回隔离页面会话，具体顺序见[段评流程](../workflows/review-flow.md)。`JS-004`、`REVIEW-001` 与读取组合测试必须能区分合法空评论、规则失败、缺失宿主能力和旧式会话错误。
