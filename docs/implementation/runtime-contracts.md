...
**当前实现**以 `RequestPlan` / `RequestBudget`（`runtime/contracts.ts`）与 `RuntimeResult`（`workflows/types.ts`）为准；下文「结果与错误」中的 `OperationResult`、`DomainChange`、`effects`/`cleanup` 属于**目标设计**，尚未在 `packages/` 导出，差异见 [已知差异](../divergence/known-divergences.md)。
...
`value` 可以是成功空列表，但不能用空列表表示失败；空结果必须使用 `empty`。当前实现不包含 `stale` / `unknown` 终态，也不返回 `effects`、`changes`、`cleanup` 字段；应用自行负责持久化提交。

### 目标设计：OperationResult

历史目标契约（**尚未实现**）曾约定更宽的统一信封，保留如下供适配层与后续实现对照：

所有公开流程目标为返回 `OperationResult<T>`，其中 `value` 可以是成功空列表，但不能用空列表表示失败：
...
```text
source-core
  -> RequestPlan / RuntimeResult / WorkflowDiagnostic（目标设计另有 OperationResult）
  -> WorkflowPorts / ReadingPorts（source-node 提供 SourceRuleHost、SourceRequestHost 等实现）
  -> 应用自行提交持久化数据
```
