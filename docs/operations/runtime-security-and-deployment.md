...
| 脚本/正则 CPU 耗尽 | 可终止执行环境和总预算；主事件循环同步正则不能靠 Promise 超时中断。书源可控正则另有一层形态守卫（`compileSourcePattern`）：长度上限 2048 字符，未锚定的嵌套量词（`(a+)+`、`(.*)*`、`(\w+\s?)+`）一律拒绝——与输入规模无关；带字面量锚定的重复（如语料里的 `(\n.*)+`）放行 | timeout、budget-exceeded，或规则以明确诊断失败 |
